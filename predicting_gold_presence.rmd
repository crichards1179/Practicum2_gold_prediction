...
str(imported)
```
Here I want to see how the totals of each mineral so I build a vector of sums of each mineral mined
...
It looks like Gold, Silver, and Lead had the most observations
```{r}
totals <- data.frame(minerals, mined)
totals
```

I transpose the data frame
```{r}
...
## Visualizations  
Here is the count from above visualized

```{r}
barplot(totals$mined, names.arg = totals[,1], cex.names = 0.5, las = 2)
```
```{r}
totalObsMined <- sum(totals$mined)
other <- totalObsMined - htot$Gold - htot$Silver - htot$Lead
portions <- c(other, htot$Gold, htot$Silver, htot$Lead)
...
I write out corrdf and import factoredCorrdf.csv for several different mineral combination factors
```{r}
...
```{r}
test1 <- factCorr
rownames(test1) <- test1$MineralType
test1
...
```{r}
set.seed(123)
km.res <- kmeans(corrdf, 10, nstart = 403)
km.res$cluster
```

```{r}
#View(corrdf)
result  <- kmeans(corrdf, 7)
...
```
```{r}
mineralAuAg <- lm(Gold ~ Silver, data = factCorr)
plot(mineralAuAg, col = mineralAuAg$Silver)
abline(mineralAuAg, col = "blue")
```
```{r}
mineralAuPb <- lm(Gold ~ Lead, data = factCorr)
plot(mineralAuPb)
