...
Here is how the top  three minerals compare to all the others added up. Total ocurrance of minerals.
...
```
```{r}
table(train$targetMineral)
```
```{r}
outcome <- predict(svmModel, train)
table(Predicted = outcome, Tagged = train$targetMineral)
```
...
### Logistic regression  
I use logistic regression to predict gold from presence of silver and lead. I also use logistic regression to examine the p-value of silver and lead to gold.
```{r}
tabImp <- imported[7:38]
